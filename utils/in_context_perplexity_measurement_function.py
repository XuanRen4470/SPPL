...
import torch
def calibrated_perplexity_with_entropy_threshold_calculation(token_prob_list, token_entropy_list, threshold, clip = 'higher'):
    perplexity_list = []
...
def probability_calculation_(probability_list):
    sum_prob = 0
    sum_token_lenth = 0
    sum_ppl = 0
    sum_calibrated_ppl = 0
    sum_calibrated_prob = 0
    for problem_id, prob_list_item in enumerate(probability_list):
        tensor = torch.tensor(prob_list_item)
        clamped = torch.clamp(tensor, min=0.001)
        calibrated_log_prob = sum(torch.log(torch.tensor(clamped)))

        log_prob = sum(torch.log(torch.tensor(prob_list_item)))
        sum_prob += log_prob
        sum_calibrated_prob += calibrated_log_prob
        sum_token_lenth += len(prob_list_item)
        ppl_item = torch.exp(-log_prob/len(prob_list_item))
        sum_ppl += ppl_item
        calibrated_ppl_item = torch.exp(-calibrated_log_prob/len(prob_list_item))
        sum_calibrated_ppl += calibrated_ppl_item
    avg_ppl = sum_ppl / len(probability_list)
    avg_calibrated_ppl = sum_calibrated_ppl / len(probability_list)
    avg_prob = (sum_prob / len(probability_list))
    avg_token_lenth = sum_token_lenth / len(probability_list)
    avg_prob = avg_prob.cpu().item()
    return avg_prob, avg_token_lenth, avg_ppl, avg_calibrated_ppl
