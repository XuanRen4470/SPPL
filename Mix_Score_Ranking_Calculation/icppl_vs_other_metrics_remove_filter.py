...
from utils.data_recorder import write_to_table, write_to_table_comparison
...
# rank_method_list = ['perplexity', 'in_context_perplexity', 'ours_filter_removed']
...
# default_lr_task_name_list = ['gsm8k', 'math_algebra']

# default_lr_task_name_list = ['plan_bench_generation', 'plan_bench_optimality', 'plan_bench_generalization', 'plan_bench_reuse', 'plan_bench_replaning', 'plan_bench_verification']#, 'plan_bench_execution']


# 'plan_bench_execution'
# default_lr_task_name_list = ['plan_bench_verification']

# default_lr_task_name_list = ['plan_bench_optimality']#, 'plan_bench_verification']#

# default_lr_task_name_list = ['gsm8k', 'math_algebra', 'mmlu', 'winogrande', 'piqa', 'agieval', 'squad', 'ecqa', 'boolq', 'arc_challenge', 'mmlu_pro_law', 'drop', 'hellaswag', 'mbpp', 'api_bank', 'mmlu_moral_scenarios', 'math_geometry', 'plan_bench_generation', 'plan_bench_optimality', 'plan_bench_generalization', 'plan_bench_reuse', 'plan_bench_replaning', 'plan_bench_execution', 'plan_bench_verification']#

model_name_list = ['mistral', 'llama_3_instruct', 'qwen']
# model_name_list = ['mistral', 'llama_3_instruct', 'qwen']
# model_name_list = ['llama_3_instruct']
...
# diff_threshold_list = [0.01]

# diff_threshold_list = [0.04]

diff_threshold_list = [-1111]

# diff_threshold_list = [0.02]
...
# index_range_list = [(50, 100)]
# index_range_list = [(100, 150)]
# index_range_list = [(0, 10), (10, 20), (20, 30), (0, 30), (30, 60), (60, 90)]
...
# # def calculate_diversity_plan_bench(data, mode: str = "sd"):
...
#     # values = np.array(
#     #     [float(v) for k, v in data.items() if k != "rewrite_groundtruth_in_own_words"], dtype=float
#     # )
...
for kk, diff_threshold in enumerate(diff_threshold_list):
    ...
    for initial_index, last_index in index_range_list:
        ...
        for ranking_method in rank_method_list:
            if ranking_method == 'in_context_perplexity':
                experimental_result['Ours'] = {}
            elif ranking_method == 'ours_filter_removed':
                experimental_result['Ours Filter Removed'] = {}
            else:
                experimental_result[ranking_method] = {}
        experimental_result['Ours - Perplexity'] = {}
        experimental_result['Ours Filter Removed - Perplexity'] = {}
        ...
        for ranking_method in rank_method_list:
            ...
            for model_name in model_name_list:
                ...
                for task_name in task_name_list:
                    ...
                    if 'plan_bench' in task_name:
                        file_filter_removed_path = f"{HOME_DIRECTORY}/Mix_Score_Ranking_Calculation/Mix_Score_record/record_book/300_icppl_{model_name}_{task_name}_{initial_index}_{last_index}_plan_bench_{num_of_incontext_examples}_examples_remove_correctness_filter.json"
                    else:
                        file_filter_removed_path = f"{HOME_DIRECTORY}/Mix_Score_Ranking_Calculation/Mix_Score_record/record_book/300_icppl_{model_name}_{task_name}_{initial_index}_{last_index}_main{suffix_}_remove_correctness_filter.json"

                    with open(file_filter_removed_path, 'r') as f:
                        icppl_filter_removed_record = json.load(f)
                    ...
                    if diversity > diff_threshold:
                        ...
                        in_context_ppl_filter_removed_values, _ = loading_icppl_scores(task_name, icppl_filter_removed_record)
                        ...
                        ranked_method_name_list, method_names_with_ranks = rank_on_metrics_all(in_context_ppl_values, in_context_ppl_filter_removed_values, ppl_values, IDF_values, log_propability_values, skywork_reward_score_values, CAR_score_values, name_list, ranking_method = ranking_method)
                ...
                if ranking_method == 'in_context_perplexity':
                    ...
                    experimental_result['Ours Filter Removed'][model_name] = calc_avg(avg_mix_score_per_model)
                    try:
                        experimental_result['Ours Filter Removed - Perplexity'][model_name] = calc_avg(avg_mix_score_per_model) - perplexity_avg_accuracy_per_model_dict[model_name]
                    except:
                        a = 1
                else:
            ...
            if ranking_method == 'in_context_perplexity':
                ...
                experimental_result['Ours Filter Removed']['Avg Acc'] = avg_mix_score
                try:
                    experimental_result['Ours Filter Removed - Perplexity']['Avg Acc'] = avg_mix_score - perplexity_avg_accuracy
                    experimental_result['Ours Filter Removed - Perplexity']['Weighted Spearman Pho'] = weighted_rho - perplexity_avg_pho
                except:
                    a = 1
                experimental_result['Ours Filter Removed']['Weighted Spearman Pho'] = weighted_rho
            else:
...
try:
    ...
    print('Ours - Perplexity: Avg Acc', experimental_result['Ours - Perplexity']['Avg Acc']* 100)
    ...
    print('Ours Filter Removed - Perplexity: mistral', experimental_result['Ours Filter Removed - Perplexity']['mistral'] * 100)
    print('Ours Filter Removed - Perplexity: llama_3_instruct', experimental_result['Ours Filter Removed - Perplexity']['llama_3_instruct']* 100)
    print('Ours Filter Removed - Perplexity: qwen', experimental_result['Ours Filter Removed - Perplexity']['qwen']* 100)
    print('Ours Filter Removed - Perplexity: Avg Acc', experimental_result['Ours Filter Removed - Perplexity']['Avg Acc']* 100)
    print('Ours Filter Removed - Perplexity: Weighted Spearman Pho', experimental_result['Ours Filter Removed - Perplexity']['Weighted Spearman Pho'])
except:
    a = 1
# write_to_table(experimental_result_list, table_tex_name)
write_to_table_comparison(experimental_result_list, table_tex_name)

# 'icppl vs other metrics: average of 3 subsets'